  ...
  -> utils::error::Result<ocppi::runtime::config::types::Config>
{
    ...
    for (const auto &mount : mounts) {
        ...
        for (auto &path : tmpfsPath) {
            if (auto common = commonParent(existsPath, path); common != originalRootPath) {
                newTmp = false;
                tmpfsPath.insert(common);
                break;
            }
        }

        if (newTmp) {
            tmpfsPath.insert(existsPath);
        }
    }
    ...
    for (const auto &tmpfs : tmpfsPath) {
        ...
        auto dir = QDir{ originalRoot.filePath(tmpfs.mid(1)) };
        for (const auto &rootDest :
            ...
            pos = mounts.insert(
              pos,
              { .destination = rootDestPath.mid(originalRoot.absolutePath().size()).toStdString(),
                .options = { { "rbind", "ro" } },
