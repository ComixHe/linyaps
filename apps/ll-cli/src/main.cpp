...
You can report bugs to the linyaps team under this project: https://github.com/OpenAtom-Linyaps/linyaps/issues)"));
    ...
    // add flags
    auto *versionFlag = commandParser.add_flag<const std::string>("--version", _("Show version"));
    auto *noDBusFlag =
      commandParser
        .add_flag<const std::string>(
          "--no-dbus",
          _("Use peer to peer DBus, this is used only in case that DBus daemon is not available"))
        ->group(CliHiddenGroup);
    auto *jsonFlag =
      commandParser.add_flag<const std::string>("--json", _("Use json format to output result"));
...
ll-cli install stable:org.deepin.demo/0.0.0.1/x86_64
    ...
    [[maybe_unused]] auto *pruneFlag =
      cliUninstall->add_flag<const std::string>("--prune", _("Remove all unused modules"))
        ->group(CliHiddenGroup);
    [[maybe_unused]] auto *allFlag =
      cliUninstall->add_flag<const std::string>("--all", _("Uninstall all modules"))
        ->group(CliHiddenGroup);
