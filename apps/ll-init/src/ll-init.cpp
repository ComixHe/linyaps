...
constexpr std::array<int, 12> unblock_signals{ SIGABRT, SIGBUS,  SIGFPE,  SIGILL,  SIGSEGV, SIGSYS,
...
public:
    ...
    bool block_signals() noexcept
    {
        ...
        // background process will be suspended by SIGTTIN and SIGTTOU if it try to read/write
        // from/to terminal, so we need to ignore these signals.
        struct sigaction ignore{};
        ignore.sa_handler = SIG_IGN;
        ignore.sa_flags = 0;

        if (::sigaction(SIGTTIN, &ignore, &ttin_action) == -1) {
            print_sys_error("Failed to ignore SIGTTIN");
            return false;
        }

        if (::sigaction(SIGTTOU, &ignore, &ttou_action) == -1) {
    ...
    [[nodiscard]] bool restore_signals() const noexcept
    {
        ...
        ret = ::sigaction(SIGTTIN, &ttin_action, nullptr);
        if (ret == -1) {
            print_sys_error("Failed to restore SIGTTIN action");
            return false;
        }

        ret = ::sigaction(SIGTTOU, &ttou_action, nullptr);
        if (ret == -1) {
...
public:
    ...
    ~file_descriptor_wrapper() noexcept { this->close(); }
...
{
    auto *appIDenv = ::getenv("LINGLONG_APPID");
    if (appIDenv == nullptr) {
        print_info("LINGLONG_APPID is not set");
        return {};
    }
    const std::string_view appID{ appIDenv };

    struct sockaddr_un addr{};
    addr.sun_family = AF_UNIX;
    addr.sun_path[0] = 0;

    constexpr size_t ABSTRACT_PATH_MAX = sizeof(addr.sun_path) - 1;
    const auto *begin = appID.begin();
    const auto *end = appID.end();
    if (appID.size() > ABSTRACT_PATH_MAX) {
        end = begin + ABSTRACT_PATH_MAX;
    }

    std::copy(begin, end, &addr.sun_path[1]);
    addr.sun_path[1 + end - begin] = 0;

    return std::make_pair(addr, offsetof(sockaddr_un, sun_path) + 1 + end - begin);
}

file_descriptor_wrapper create_abstract_socket() noexcept
{
    auto fd = ::socket(AF_UNIX, SOCK_NONBLOCK | SOCK_SEQPACKET, 0);
    file_descriptor_wrapper socket_fd{ fd };
    if (fd == -1) {
        print_sys_error("Failed to create abstract socket");
        return socket_fd;
    }
...
file_descriptor_wrapper start_timer(const file_descriptor_wrapper &epfd) noexcept
{
    ...
    if (interval != nullptr) {
        try {
            ...
            print_info("Invalid interval, using default 5 seconds");
...
int delegate_run(const std::vector<std::string> &args) noexcept
{
    auto child = fork();
    if (child == -1) {
    ...
    if (child == 0) {
        ...
        if (grandchild == -1) {
            print_sys_error("Failed to fork grandchild");
            return -1;
        }
...
bool handle_client(const file_descriptor_wrapper &unix_socket) noexcept
{
...
int main(int argc, char **argv) // NOLINT
{
    ...
    auto unix_socket = create_abstract_socket();
    if (!unix_socket) {
        return -1;
    }
