...
int mountSelfBundle(const lightElf::currentElf &elf,
                    const linglong::api::types::v1::UabMetaInfo &meta) noexcept
{
    ...
    auto bundleOffset = bundleSh->offset;
    if (auto digest = calculateDigest(elf.underlyingFd(), bundleOffset, bundleSh->size);
        digest != meta.digest) {
...
std::optional<linglong::api::types::v1::UabMetaInfo> getMetaInfo(const lightElf::currentElf &elf)
{
    ...
    std::string content(metaSh->size, '\0');
    if (::pread(elf.underlyingFd(), content.data(), metaSh->size, metaSh->offset) == -1) {
        std::cerr << "read failed:" << ::strerror(errno) << std::endl;
        return {};
    }
...
int main(int argc, char **argv)
{
    ...
    lightElf::currentElf elf(selfBin);
