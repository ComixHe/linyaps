...
utils::error::Result<bool> Cli::reuseContainer(const package::Reference &appRef,
...
int Cli::run(const RunOptions &options)
{
    ...
    auto cliLockRet = utils::filelock::FileLock::create(common::dir::getRuntimeDir() / lock_name);
    if (!cliLockRet) {
    ...
    auto lockResult = containerLock.tryLock(utils::filelock::LockType::Write);
    if (!lockResult) {
