...
prepareSymlink(const std::filesystem::path &sourceRoot,
               ...
               long maxCount) noexcept
{
    LINGLONG_TRACE("prepare symlink")

    auto source = sourceRoot / sourceFile;
    auto destination = destinationRoot / destinationFile;
    ...
    while (status.type() == std::filesystem::file_type::symlink && maxCount > 0) {
        ...
        if (target.is_relative()) {
            newSource = source.parent_path() / target;
        } else {
            newSource = sourceRoot / target.lexically_relative("/");
        }
        newSource = std::filesystem::weakly_canonical(newSource, ec);

        if (ec) {
            return LINGLONG_ERR(
              QString{ "canonicalizing new source %1 failed: %2" }.arg(newSource.c_str(),
                                                                       ec.message().c_str()));
        }

        source = std::move(newSource);
        destination = destinationRoot / source.lexically_relative(sourceRoot);
        // ensure parent directory of destination after destination has been updated
        if (!std::filesystem::create_directories(destination.parent_path(), ec) && ec) {
            return LINGLONG_ERR(
              "couldn't create directories "
              % QString::fromStdString(destination.parent_path().string() + ":" + ec.message()));
        ...
        status = std::filesystem::symlink_status(source, ec);
        if (ec) {
            // previous symlink is broken, just break
    ...
    for (const auto &layer : std::as_const(this->layers)) {
        ...
        if (!files.empty()) {
            ...
            for (std::filesystem::path source : files) {
                ...
                auto ret =
                  prepareSymlink(basePath, sourceFile, moduleFilesDir, sourceFile, symlinkCount);
                if (!ret) {
                    return LINGLONG_ERR(ret);
                }
                ...
                std::filesystem::create_hard_link(source, realDestination, ec);
                if (ec) {
        ...
        if (base) {
            ...
            for (std::filesystem::path file : this->neededFiles) {
                ...
                const auto &[source, destination] = *ret;
                ...
                std::cout << "copy file: " << source.string() << " to " << destination.string()
                          << std::endl;
